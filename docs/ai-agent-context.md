...
- **Places**: `/api/places`, `/api/places/:id`, `/api/places/status`, `/api/places/:id/categories`, `/api/places/:id/rating`, `/api/places/:id/ranking` (service token auth via `DATA_TEAM_AUTH_TOKEN` env var) (POST `/api/places` accepts array of place IDs in body)
