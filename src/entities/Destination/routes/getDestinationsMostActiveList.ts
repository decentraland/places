...
export const getDestinationsMostActiveList = Router.memo(
  async (ctx: Context<{}, "url" | "request">) => {
    ...
    const hotScenesPositions = hotScenes
    ...
    const [data, total, sceneStats] = await Promise.all([
      PlaceModel.findDestinationsWithHotScenes(enhancedOptions, hotScenes),
