...
export function createPlaceFromDeployment(deployment: ContentDepoymentScene, data: Partial<Omit<PlaceAttributes, 'id'>> = {}): PlaceAttributes {
  const now = new Date()
  ...
  if (thumbnail && !thumbnail.startsWith('https://')) {
    const content = deployment.content.find(content => content.key === thumbnail)
    if (!content || content.hash === 'bafkreihdwdcefgh4dqkjv67uzcmw7ojee6xedzdetojuzjevtenxquvyku') {
      thumbnail = null
    } else {
  ...
  return {
      ...
      disabled: false,
      disabled_at: !!data.disabled && !data.disabled_at ? now : data.disabled_at || null ,
