  ...
  test(`should return a list of places matching the parameters FindWithAggregatesOptions without user`, async () => {
    ...
    expect(sql.text.trim().replace(/\s{2,}/gi, " ")).toEqual(
      `
        ...
        ORDER BY p.like_rate DESC NULLS LAST, p."deployed_at" DESC
          LIMIT $2
          OFFSET $3
      `
        .trim()
  ...
  test(`should return a list of places matching the parameters FindWithAggregatesOptions with limit of more than 100`, async () => {
    ...
    expect(sql.text.trim().replace(/\s{2,}/gi, " ")).toEqual(
      `
        ...
        ORDER BY rank DESC,
          p.like_rate DESC NULLS LAST, p."deployed_at" DESC
          LIMIT $5
          OFFSET $6
      `
        .trim()
    ...
    expect(sql.text.trim().replace(/\s{2,}/gi, " ")).toEqual(
      `
        ...
        UPDATE "places"
          ...
          "like_rate" = (CASE WHEN (c.count_active_likes + c.count_active_dislikes > 0) THEN 
    ...
    expect(sql.text.trim().replace(/\s{2,}/gi, " ")).toEqual(
      `
        ...
        WHERE
          ...
          AND world_name IN ($4)
          AND rank > 0
        ORDER BY p.like_rate DESC NULLS LAST, p."deployed_at" DESC , rank DESC
          LIMIT $5
          OFFSET $6
      `
        .trim()
