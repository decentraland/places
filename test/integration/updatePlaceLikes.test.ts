...
describe("when updating likes on a place via PATCH /places/:entity_id/likes", () => {
  beforeAll(async () => {
    await initTestDb()
  })
  ...
  describe("and the place does not exist", () => {
    ...
    it("should respond with a 404", async () => {
      const response = await supertest(app)
        .patch(`/api/places/${entityId}/likes`)
        .send({ like: true })
  ...
  describe("and the place exists", () => {
    ...
    describe("and the user likes the place", () => {
      it("should persist the like and return the updated counts", async () => {
        const response = await supertest(app)
          .patch(`/api/places/${place.id}/likes`)
          .send({ like: true })
          .expect(200)
    ...
    describe("and the user dislikes the place", () => {
    ...
    describe("and the likes count is persisted in the database", () => {
      ...
      it("should reflect the updated counts when fetching the place", async () => {
        const response = await supertest(app)
